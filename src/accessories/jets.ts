...
export class JetsAccessory {
  ...
  constructor(
    ...
    this.service.getCharacteristic(this.C.On)
      .onGet(() => this.platform.statusCache.get()?.jet_state === 1)
      .onSet((v: CharacteristicValue) =>
        this.platform.sendCommandAndPoll({ jet_state: (v ? 1 : 0) as 0 | 1 }));

    platform.statusCache.subscribe((s: MspaDeviceStatus) =>
      this.service.updateCharacteristic(this.C.On, s.jet_state === 1));
  }
}
